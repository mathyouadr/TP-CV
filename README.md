...
| pagespeed.web.dev (mobile) | **Performance : 99 / Accessibilité : 100 / Bonnes pratiques : 100 / SEO : 100** (Navigation agentique : 2/2)<br>First Contentful Paint : 0,8 s · Largest Contentful Paint : 0,8 s · Total Blocking Time : 0 ms · Cumulative Layout Shift : 0 · Speed Index : 3,5 s<br>*Dernière mesure le 16/09 à 18h04 (après la correction 16), Lighthouse 13.4.1, Moto G Power émulé en 4G lente. En version ordinateur : 100 partout.* | Page très légère (environ 32 ko, sans image, police ni bibliothèque externe) : contenu affiché en 0,8 s, sans décalage de mise en page. Pistes signalées :<br>• **Animation non composée** : corrigée (correction 16), l'audit est maintenant réussi.<br>• **CSS bloquant le rendu (300 ms estimés)** : conservé, car l'arborescence demandée impose un fichier `css/style.css` séparé.<br>• **Durée de cache (6 Kio)** : fixée par GitHub Pages, non modifiable.<br>• Les en-têtes de sécurité non notés (CSP, HSTS, COOP…) sont gérés par GitHub Pages.<br>Le Speed Index (3,5 s) reste plus élevé que le First Contentful Paint : je pensais que l'animation en était la cause, mais le test après correction montre que non (3,6 s → 3,5 s). Il vient de la simulation 4G lente et n'empêche pas le score de 99. |
...
| 16 | PageSpeed signalait une animation non composée : le point vert du bandeau animait son ombre (`box-shadow`) en boucle. | Animation supprimée, le point reste fixe avec un léger halo. | C6 | Audit « Éviter les animations non composées » réussi au test du 16/09 à 18h04. Score Performance inchangé (99). |
...
| C7 Validation & corrections | 4 | 10 | 40 | W3C sans erreur, structure Hn validée par Outiref, PageSpeed mobile entre 99 et 100, journal de 16 corrections. |
